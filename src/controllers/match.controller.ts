...
const startMatchByIdController: RequestHandler<{
  id: string;
}> = async (req, res) => {
  const valid = matchStartSchema.safeParse(req.body);

  if (!valid.success) {
    throw valid.error;
  }

  const { data, maxPoints } = valid.data;

  const match = await updateMatchById(req.params.id, {
    maxPoints,
    comments: [data],
  });
